...
func TestRuleAnd(t *testing.T) {
	...
	t.Run("between two Over indicators", func(t *testing.T) {
		...
		rule1 := NewAndRule(rule1True, rule2True, ruleCache)
		rule2 := NewAndRule(rule1True, rule3False, ruleCache)
		rule3 := NewAndRule(rule3False, rule2True, ruleCache)
		...
		g.Expect(rule3.IsSatisfied(start.Unix())).To(BeFalse())

	})

	t.Run("test the AND()", func(t *testing.T) {
		g := NewGomegaWithT(t)
		history := NewPerformanceAskBidTradeHistory()
		cache := NewIndicatorCache(history)
		ruleCache := NewRuleCache(history)
		...
		rule1 := rule1True.And(rule2True)
		rule2 := rule1True.And(rule3False)
		rule3 := rule3False.And(rule2True)
